...
- Variant-aware features should pass an attribute set with `variant` to `configFiles`, `homeFiles`, `configDir`, or `homeDir`; files without a matching `##variant.<name>` / `##v.<name>` alternate fall back to the base file. Use `exclude` with directory helpers only when a path should not be deployed at all.
