...
- The Intel macOS host `dinosaur` uses the `"26.05"` channel defined in `flake.nix`'s `series` attrset. The channel name surfaces to modules as `settings.nixpkgsSeries = "26.05"`.
...
- `pomelo` (Fedora), `pterosaur` (Ubuntu Server 26.04 LTS), and `mango` (Ubuntu Desktop 26.04 LTS) are home-manager-only; they have no system `configuration.nix`.
- Flake output names do not always match host directories: `orb-aarch64 -> orbstack` and `vm-aarch64 -> vmfusion` are intentional.
- `dinosaur` is pinned to the `"26.05"` series because nixpkgs-unstable no longer supports `x86_64-darwin`.
