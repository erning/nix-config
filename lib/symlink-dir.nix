{ src, dst, prefix, mkSymlink, host }:

let
  parseAlternate = name:
    let m = builtins.match "(.+)##(.+)" name;
    in if m == null then { base = name; condition = null; }
       else { base = builtins.elemAt m 0; condition = builtins.elemAt m 1; };

  isHostMatch = condition:
    condition == host
    || condition == "h.${host}"
    || condition == "hostname.${host}";
  ...
  readDirRec = base: rel:
    let
      ...
      hostMatched = builtins.foldl' (acc: name:
        let p = parseAlternate name;
        in if entries.${name} != "directory"
              && p.condition != null
              && isHostMatch p.condition
           then acc // { ${p.base} = true; }
           else acc
      ) {} names;
    in
